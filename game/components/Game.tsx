...
const COIN_INTERVAL_MS   = 30000;       // 30 seconds between coins
...
h, 4, PAL.cyan);
    ...
    // Low ceiling — must duck... but we removed slide. Player must jump through gap.
    px(ctx, 0, 0, obs.width, obs.height, PAL.spikeDk);
    px(ctx, 0, obs.height - 4, obs.width, 4, PAL.spike);
    // Hanging spikes
    for (let i = 0; i < Math.floor(obs.width / 20); i++) {
      ctx.fillStyle = PAL.spike;
      ctx.beginPath();
      ctx.moveTo(i * 20 + 2, obs.height);
      ctx.lineTo(i * 20 + 10, obs.height + 12);
      ctx.lineTo(i * 20 + 18, obs.height);
      ctx.closePath(); ctx.fill();
    }
  }
    ...
    const loop = (timestamp: number) => {
      ...
      obstaclesRef.current = obstaclesRef.current.filter(obs => {
        if (state.isRunning) {
          obs.x -= state.speed;
          // Rotate wheels
          if (obs.type === 'rotating_wheel') {
            obs.angle = (obs.angle || 0) + 0.05 + state.speed * 0.005;
          }
          // Move moving spikes
          if (obs.type === 'moving_spike' && obs.moveDir !== undefined) {
            obs.y += obs.moveDir * 2;
